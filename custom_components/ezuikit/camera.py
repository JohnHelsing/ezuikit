...
#        self.schedule_update_ha_state()
    ...
    def get_device_battery(self):
        r = requests.post('https://open.ys7.com/api/lapp/device/status/get', data={'accessToken':self.accessToken,'deviceSerial':self.deviceSerial})
        result = r.json()
        if (result['code']=='200'):
            return result['data']['battryStatus']
        else:
            return 'error'
